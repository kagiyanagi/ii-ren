...
AbstractWidget {
    ...
    onPressed: mouse => {
        if (mouse.button === Qt.RightButton && !isPreview) {
            GlobalStates.desktopMenuWidgetId = widgetInstance ? widgetInstance.id : configEntryName;
            GlobalStates.desktopMenuScreen = root.Window.window?.screen ?? Quickshell.screens[0];
            const gp = root.mapToItem(null, mouse.x, mouse.y);
            GlobalStates.desktopMenuX = gp.x;
            GlobalStates.desktopMenuY = gp.y;
            GlobalStates.desktopMenuOpen = true;
            return;
        }
        beginPointerGesture(mouse)
    }
