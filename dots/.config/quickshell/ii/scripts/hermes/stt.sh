#!/usr/bin/env bash
# Runs scripts/hermes/stt.py inside the Hermes agent's own interpreter, with the
...
[[ -d $agent_dir ]] || { echo '{"ok":false,"text":"","error":"hermes-agent not found"}'; exit 127; }
...
if [[ -x $agent_dir/venv/bin/python ]]; then
    ...
    [[ -n $python_bin ]] || { echo '{"ok":false,"text":"","error":"no python for hermes-agent"}'; exit 127; }
fi

cd "$agent_dir"
unset PYTHONPATH PYTHONHOME
export HERMES_HOME="$hermes_home"
exec "$python_bin" "$here/stt.py" "$@"
